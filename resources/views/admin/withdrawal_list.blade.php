...
@section('content')
                ...
                <div class="top-bar">
                    <!-- BEGIN: Breadcrumb -->
                    <div class="-intro-x breadcrumb mr-auto hidden sm:flex"> <a href="" class="">Admin</a> <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Withdrawals</a> </div>
                    <!-- END: Breadcrumb -->
                    <!-- BEGIN: Search -->
...
<div class="grid grid-cols-12 gap-6 mt-1">
                    ...
                    <table class="table table-report table-report--bordered display datatable w-full" style="font-size:14px;">
                        <thead>
                            <tr>
                                ...
                                <th class="border-b-2 whitespace-no-wrap">RIDER</th>
                                <th class="border-b-2 whitespace-no-wrap">WITHDRAWAL</th>
                               
                                <th class="border-b-2 whitespace-no-wrap">BALANCE</th>
                        ...
                        <tbody>
                          @foreach($tickets as $customer) 
                            <tr>
                          
                           
                                <td class="">
                                <div class="font-medium whitespace-no-wrap" >{{$customer->firstname}}</div>
                                  
                                </td>
                                <td class="">
                                <div class="font-medium whitespace-no-wrap" >{{$customer->amount}}</div>
 ...
 <td class="w-40">
                                ...
                                <div class="mt-4">
                                   <?php  if($customer->status=="new"){?>
                                 <span class="px-3 py-2 rounded-full bg-theme-6 text-white mr-1">W.Request</span>
                                 
                                 <?php }?>
                                 <?php  if($customer->status=="ok"){?>
                                 <span class="px-3 py-2 rounded-full bg-theme-9 text-white mr-1">Approved</span>
                               ...
                               <td class="w-40">
                                ...
                                <div class="flex sm:justify-center items-center">
                                    ...
                                    <?php  if($customer->status!="ok"){?>
                                  <a class="button button--sm w-8 mr-1 mb-2 bg-theme-7 text-white" id="deleteUser{{ $customer->id}}" data-userid="{{ $customer->id}}" href="javascript:void(0)" onclick="showAlert({{$customer->id}});" > <i data-feather="external-link" class="w-4 h-4 mr-1"></i></a>
               ...
               {{ csrf_field() }}
                    ...
                    <div class="modal__content">
                        <div class="p-5 text-center">
                            ...
                            <i data-feather="help-circle" class="w-16 h-16 text-theme-6 mx-auto mt-3"></i> 
                            <h2 style="font-weight:bold">Do you really want to Approve this withdrawer ticket?</h2>
                            <div class="p-5">
                            ...
                            <a href="" class="block font-medium text-base mt-5" style="color:#090"><span id="firstname" >agaga robinson</span></a>
                            <span id="id"></span>
                            <a href="" class="block font-medium text-base mt-5"><b><span id="" >WITHDRAW:</span></b> NGN <span id="amount">678,000</span></a> 
                             <a href="" class="block font-medium text-base mt-5"><b><span id="" >BALANCE:</span></b> NGN <span id="balance">5,000</span></a> 
                              <a href="" class="block font-medium text-base mt-5"><b><span id="" >DATE:</span></b> <span id="date">5,000</span></a> 
                        ...
                        <div class="px-5 pb-8 text-center">
                            ...
                            <button type="submit" class="button w-26 bg-theme-9 text-white" onclick="senddel();" >Yes Approve !</button>
                        </div>
                    </div>
                </div>
                </form>
...
function showAlert(photo){
    ...
    $.get('/admin/pullwithdrawdetail/'+id,function(detail){
     $("#firstname").text(detail.firstname.toUpperCase());
     $("#amount").text(detail.amount);
     $("#balance").text(detail.balance);
      $("#date").text(detail.created_at);
...
function senddel(){
    ...
    window.location="/admin/approve-ticket/"+$('#app_id').val();
